...
rely on tmux 3.x features (``if-shell -F``, ``extended-keys``, pane-scoped
options), and the quiet ``-q`` option flags mean an ancient tmux (e.g. the
1.8 that CentOS 7 ships) degrades silently and fails late in cryptic ways
instead of being flagged up front.
...
# An ancient tmux comfortably below any plausible supported floor (the
# repo's own pane-integration gate is 3.2; CentOS 7 shipped 1.8).
...
def _setup_pre_menu_output(tmp_path: Path, version_line: str) -> str:
    ...
    for key in list(env):
        if key.startswith(("ANTHROPIC", "OPENAI_", "GEMINI", "OMNIGENT_RUNNER")):
            env.pop(key)
    child = pexpect.spawn(
        sys.executable,
        ["-m", "omnigent", "setup"],
...
def test_setup_warns_on_outdated_tmux(tmp_path: Path) -> None:
    ...
    assert "tmux" in text.lower(), (
        "`omnigent setup` printed no tmux warning although the tmux on PATH "
...
def test_setup_quiet_on_modern_tmux(tmp_path: Path) -> None:
    ...
    assert "tmux" not in text.lower(), (
        "`omnigent setup` warned about tmux although the tmux on PATH "
